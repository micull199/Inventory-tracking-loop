...
**Last commit:** _(pending — S2.5 will land below)_
**Branch:** main
**Tests:** `make check` green: ruff ✓, mypy ✓ (11 files), pytest 204 unit+integration ✓, Playwright 6 e2e ✓.
**Definition-of-Done items ticked:** 0 / 12. S2.5 is a refactor — no DoD item advances. Behaviourally identical to the end of iteration 7.

**Repo health:**
- ruff: clean (`app tests`)
- mypy: clean (strict on `app/`, 11 source files — `app/template_env.py` added)
- pytest unit + integration: 204 passing (was 198, +6 from `tests/unit/test_template_env.py`)
- Playwright e2e: 6 passing (unchanged — refactor, no new behaviour)

**What's running:** Same set of routes and templates as end of S2. The change is structural: `app/template_env.py` is now the single source for the shared `Jinja2Templates` instance (CSRF + flash context processors registered once). `app/main.py`, `app/suppliers.py`, and `app/locations.py` import `templates` from there directly. The previous `init_templates()`/`_t()` shim and the per-router `_templates` global are gone. Behaviour, including which roles see which page and which forms write which audit rows, is unchanged.
...
*(none — slice S2.5 just shipped, see Completed log)*

---

## Next slice

**Slice name:** S3 — Taxonomy: top-level categories CRUD (Manager-owned)
**Targets DoD item(s):** #2 (Manager defines taxonomy), #8 (audit on `entity_type="taxonomy_node"`), #9 (third Manager-only route family).
**Why this next:** taxonomy is the prerequisite for items (I1+). Top-level categories first (parent_id null), sub-categories follow in S4. The shape is a third concrete instance of the settings-CRUD pattern; once it's in we'll have three routes side by side and can decide whether the route-helper duplication (`_normalise`/`_validate_name`/`_check_name_unique`/`_diff`/`_flash`) is worth collapsing into a generic helper. Don't pre-extract — wait for the third concrete shape.
