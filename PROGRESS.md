...
**Last commit:** *(see Completed slices log; commit hash filled in by the next iteration)*
**Branch:** main
**Tests:** `make check` green: ruff ✓, mypy ✓ (13 files), pytest 384 unit+integration ✓, Playwright 7 e2e ✓.
**Definition-of-Done items ticked:** 0 / 12. S5 advances DoD #2, #8, #9 but does not complete any of them — #2 still needs items (I1) + required-field enforcement at item-edit time (I2/S6); #8 and #9 stay system-wide guarantees until the full feature set is in.

**Repo health:**
- ruff: clean (`app tests`)
- mypy: clean (strict on `app/`, 13 source files — `app/field_defs.py` added)
- pytest unit + integration: 384 passing (was 308, +76 — 10 new unit `_field_defs` model tests + 66 new integration `_field_defs_routes` tests covering role enforcement, list/new/create/edit/archive/unarchive, leaf-invariant on create + on unarchive, options validation per type, key auto-derivation + collisions, and the cross-cutting "sub-cat create blocked when parent has active field defs" guard)
- Playwright e2e: 7 passing (`test_taxonomy_e2e.py` extended again — manager now drills into Silver's fields, creates a "Karat" select with options 9/14/18, archives it, switches to archived tab, unarchives)

**What's running:** Manager + Admin can define a custom-field schema on every taxonomy *leaf*. Routes (all gated by `require_role(Role.MANAGER)` + CSRF):

- `GET  /admin/taxonomy/{node_id}/fields[?show=active|archived]` — list active/archived field defs on a node, with parent context (for sub-cats) and a "Back" link to either the top-level list or the parent's children list. The list view shows a "non-leaf" note + hides the CTA when the node has active children, and an "archived node" note when the node itself is archived.
- `GET  /admin/taxonomy/{node_id}/fields/new` — form (400 if non-leaf or archived node).
- `POST /admin/taxonomy/{node_id}/fields` — create.
- `GET  /admin/taxonomy/fields/{field_id}/edit` — form (flat by id).
- `POST /admin/taxonomy/fields/{field_id}` — update.
- `POST /admin/taxonomy/fields/{field_id}/archive`
- `POST /admin/taxonomy/fields/{field_id}/unarchive` — re-checks the leaf invariant; 400 if the node is now non-leaf or archived.

**Leaf invariant** (per MISSION §3): a top-level node with no active sub-categories is a leaf; a sub-category is always a leaf. Field defs only attach to leaves; sub-cat creation under a node with active field defs is now blocked (400). Archived field defs do *not* block sub-cat creation — they're history, not schema, so the workflow is "archive defs → add sub-cats". Helper `app.field_defs.has_active_field_defs(db, node_id)` is imported by `app.taxonomy` and called from both `GET /admin/taxonomy/{parent_id}/children/new` and `POST /admin/taxonomy/{parent_id}/children`.

**Key auto-derivation**: `re.sub(r"[^a-z0-9]+", "_", name.lower()).strip("_")`. Empty after derive → 400. The key is the stable handle items will reference; renaming a field re-derives a new slug, but a casing-only rename keeps the slug intact (so item references stay valid as long as the manager doesn't change letters/digits). The audit diff records both `name` and `key` when a rename shifts the slug.

**Options validation** (`select` / `multiselect` only): textarea blob → list, one option per line, stripped, empty lines dropped, duplicates rejected (400). Non-empty list required. For non-select types, options must be empty/null (400 if any options are submitted).

**Required flag**: HTML checkbox with explicit `value="true"`. Absent in form data ⇒ false.

**sort_order**: same convention as nodes/sub-cats — blank on create steps by 10 from `max(existing on this node)` (or 0 if first); blank on edit means "leave alone".

**Audit**: `taxonomy_field_def.{created,updated,archived,unarchived}`. Created `after_json` carries `node_id, name, key, type, options_json, required, sort_order`. Updates use the `_diff` sparse-changed-fields pattern; if a name change shifts the slug, the diff also records `key`.

**Migration:** `migrations/versions/0006_create_taxonomy_field_defs.py` adds the table + four indexes (`ix_node_id`, `ix_archived_at`, `uq_node_name`, `uq_node_key`) — both unique indexes scope across active *and* archived rows.

**Templates:**
- `taxonomy_field_defs_list.html` — active/archived tabs, parent context, columns: order, name, type, required, options preview (`options_json | join(", ")`).
- `taxonomy_field_def_form.html` — name, type select, options textarea (always present, validated server-side per type), required checkbox, sort_order number.
- `taxonomy_list.html` updated: per-row "Fields" link renders only when `n.id in leaf_ids` (server-computed set in `list_taxonomy`).
- `taxonomy_children_list.html` updated: per-row "Fields" link unconditional (sub-cats are always leaves).
...
*(none — slice S5 just shipped, see Completed log)*

---

## Next slice

**Slice name:** S6 — Field-schema versioning behaviour (existing items keep values; new edits enforce current schema; deletes hide-not-purge)
**Targets DoD item(s):** #2 (required-field enforcement on item edit), #8 (audit on schema-version transitions).
**Why this next:** S5 lands the *what* of the schema (the field defs). S6 nails the *how it behaves over time*: when a manager renames or removes a field while items reference it, what do existing items look like, and what's the rule when those items are next saved? Per MISSION §3: "existing items keep their stored values, but new edits to those items must satisfy the current schema. Deleting a field hides it from new entry but preserves the value in audit history." That's a behaviour spec, not a data-shape spec — the data shape (id-and-key referenceable from `item_field_values`) gets fixed in I1/I2. **Decision: park S6 until items exist.** S6 needs item-side tests to demonstrate the behaviour (existing item with archived-field value → still readable; new item under same leaf → archived field hidden; edit existing item → required-fields-of-current-schema enforced). The slice is empty without items. Bring S6 forward only after I1 (core item create/edit) lands.

**Better next instead:** **I1 — Item core fields + create/edit/archive against a chosen leaf node.** Items are the unblocking primitive for everything in §3 from "Stock movements" onward. The skeleton is small: `items` table per MISSION §6, leaf-node picker (top-level *or* sub-cat, but only leaves), core fields (sku, name, unit, tracking_mode, current_qty, reorder threshold/qty, supplier_id, location_id, qr_code, notes), create/edit/archive routes, role-gated for Admin (creation) + Manager (settings adjacent) + Office. That's a multi-iteration slice — break it into I1a (Admin creates an item with core fields only, no custom fields, no movements), I1b (Office can edit existing items but not create), and I1c (archive/unarchive). The I1a slice alone is one iteration of work and unblocks I2 (custom fields rendered + validated against leaf schema), which is when S6's versioning rules become testable.
