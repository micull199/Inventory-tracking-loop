...
**Last commit:** cb19de3 — slice: S4 — Taxonomy: sub-categories under a category (Manager-owned)
**Branch:** main
**Tests:** `make check` green: ruff ✓, mypy ✓ (12 files), pytest 308 unit+integration ✓, Playwright 7 e2e ✓.
**Definition-of-Done items ticked:** 0 / 12. S4 advances DoD #2, #8, #9 but does not complete any of them — #2 still needs custom field schemas (S5) + items (I1) + required-field enforcement (I2/S6); #8 and #9 stay system-wide guarantees until the full feature set is in.

**Repo health:**
- ruff: clean (`app tests`)
- mypy: clean (strict on `app/`, 12 source files)
- pytest unit + integration: 308 passing (was 262, +46 — all in `test_taxonomy_routes.py` covering sub-cat role enforcement, list/create/edit/archive/unarchive, depth-limit guard, archived-parent guard, sibling-scoped uniqueness with archived-name conflict, and audit content)
- Playwright e2e: 7 passing (`test_taxonomy_e2e.py` extended with sub-cat create / archive / unarchive walk after the existing top-level walk)

**What's running:** Taxonomy CRUD now covers both levels. Top-level routes unchanged (`/admin/taxonomy[?show=…]`, `/new`, `/{id}/edit`, `/{id}` POST, `/{id}/archive`, `/{id}/unarchive`). Sub-cat routes added at two URL shapes:

- `/admin/taxonomy/{parent_id}/children[?show=…]` — list active/archived sub-cats under a parent; back-link to top-level.
- `/admin/taxonomy/{parent_id}/children/new` — sub-cat form, scoped by parent.
- `POST /admin/taxonomy/{parent_id}/children` — create.
- `/admin/taxonomy/sub/{node_id}/edit`, `POST /admin/taxonomy/sub/{node_id}`, `/sub/{node_id}/archive`, `/sub/{node_id}/unarchive` — edit/update/archive/unarchive a sub-cat by id (flat URL, no parent in path; the handler verifies `parent_id IS NOT NULL` and 404s otherwise).

Depth-limit guard (max two levels) is enforced application-side in `_get_top_level_parent`: a `POST /admin/taxonomy/{sub_cat_id}/children` returns 400. Archived-parent guard returns 400 from the new-sub-form GET and from the create POST — but archive/unarchive of *existing* sub-cats remains allowed even when the parent is archived (cleanup, not new structure). Audit vocabulary unchanged — `taxonomy_node.{created,updated,archived,unarchived}` reused for sub-cats; the `created` row's `after.parent_id` is the actual parent id (was hard-coded `None` for top-level, kept that way). Top-level list view gains a "Manage" link per row that drills into sub-categories. The `taxonomy_form.html` template now accepts an optional `parent` and `back_url` and adapts heading/CTA accordingly. New `taxonomy_children_list.html` for the per-parent list.
...
*(none — slice S4 just shipped, see Completed log)*

---

## Next slice

**Slice name:** S5 — Taxonomy: custom field defs per leaf node (text, number, decimal, date, boolean, select, multiselect)
**Targets DoD item(s):** #2 (Manager defines taxonomy + custom fields), #8 (audit on field-def mutations), #9 (role gating).
**Why this next:** S4 closes the structural side of the taxonomy (top-level + sub-cats). S5 attaches a per-leaf field schema — the data layer Items (I1+I2) needs to read in order to render and validate. The migration adds a `taxonomy_field_defs` table per MISSION §6, with the eight field types (text, number, decimal, date, boolean, single-select with options, multi-select with options) gated by a CHECK or SAEnum. Routes mirror the settings-CRUD shape: list / new / edit / archive (no unarchive yet — TBD whether deletes hide-not-purge plays the same way for field defs as for nodes). The "leaf" question (is this leaf node really a leaf, or does it have sub-cats?) needs deciding: per MISSION §3, "for each leaf node (a Category, or a Sub-category if one exists under that Category), a Manager can define a schema". That means: a node with no sub-cats can have field defs; a node with sub-cats cannot, because items live on the leaves only. The S5 plan will need to enforce that invariant — both at field-def-create time (parent must be a leaf) and at sub-cat-create time (creating a sub-cat under a node that already has field defs would un-leaf it; reject with 400 or migrate the defs).
