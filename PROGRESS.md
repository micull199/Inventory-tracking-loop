...
**Last commit:** *(set after commit lands; placeholder)* — slice: S2 — Locations CRUD (Manager-owned) (DoD #2, #8, #9)
**Branch:** main
**Tests:** `make check` green: ruff ✓, mypy ✓ (10 files), pytest 198 unit+integration ✓, Playwright 6 e2e ✓.
**Definition-of-Done items ticked:** 0 / 12. S2 advances #2 (second Manager-owned settings entity — locations join suppliers), #8 (audit on a second non-user entity, `entity_type="location"`), #9 (a second Manager-only route family verified Workshop=403 / Office=403 / Manager+Admin=200 in integration tests). Not ticking yet: #2 still needs taxonomy + items (S3+, I1+); #9 wants e2e coverage of the negative path; #8 is essentially trustworthy but the rolling self-critique note about "migration → trigger end-to-end test" remains.

**Repo health:**
- ruff: clean (`app tests`)
- mypy: clean (strict on `app/`, 10 source files — `app/locations.py` added)
- pytest unit + integration: 198 passing (was 152, +46 from this slice)
- Playwright e2e: 6 passing (was 5, +1 from this slice — manager creates / archives / unarchives a location via the UI)

**What's running:** Everything from S1, plus the **Manager-owned `/admin/locations` CRUD**: list (active/archived tabs, default active), new form, create, edit form, update with audit-row diff, archive/unarchive (idempotent — no audit row on a no-op). Schema is `locations(id, name unique, notes, archived_at, created_at, updated_at)` with an index on `archived_at`. Every mutation calls `record_audit` with `entity_type="location"`. Role gate is `require_role(Role.MANAGER)`; Workshop and Office both 403. **Nav link "Locations" added** to the role-aware primary nav for Manager + Admin (alongside Suppliers). The flash-message region introduced in S1 is reused for "Location 'X' created/updated/archived/restored." messages.
...
*(none — slice S2 just shipped, see Completed log)*

---

## Next slice

**Slice name:** S2.5 — Extract shared "settings CRUD" template-env helper *(refactor before S3 lands)*
**Targets DoD item(s):** none directly — this is structural housekeeping that keeps subsequent slices honest. Indirect impact on #10 (maintainability — "boring is good", "competent dev should be able to ship a change in under a day").
**Why this next:** S1 + S2 now share a verbatim `init_templates`/`_t` shim and a near-verbatim shape across `app/suppliers.py` and `app/locations.py` (normalise/validate/check-unique/diff/flash + six route handlers each). The S1 self-critique committed to "extract before the third router lands" — S3 (taxonomy) is next, so the threshold is now hit. Concrete plan: add `app/template_env.py` exposing the single `Jinja2Templates` instance constructed with the CSRF + flash context processors; have `app/main.py` and each router import from it directly (no more `init_templates()` plumbing). Keep route handlers as-is — the duplication of `_normalise`/`_diff`/etc. is *similar*, not *identical* (different field sets), so collapsing it is premature; revisit after S3 once the third concrete shape is on the page. Tests: existing layout + suppliers + locations test suites must continue to pass, no new test surface added.

If S2.5 turns out to be a wash (e.g. importing the templates module triggers a circular dep and the cure is worse than the disease), bail and pick **S3 — Taxonomy: top-level categories CRUD** instead, document the bail in the rolling self-critique, and eat the third copy of `init_templates`.
...
|------|-------|--------|-------|
| 7 | S2 — Locations CRUD (Manager-owned) | *(pending — set after commit lands)* | New Manager-only `/admin/locations` server-rendered CRUD: list (active/archived tabs), new, create, edit, update (with field-level audit diff), archive, unarchive. Migration `0004_create_locations.py` adds the `locations` table (`name` unique across active + archived rows). `Location` ORM model in `app/models.py`. New `app/locations.py` router uses the same `init_templates()` shim as `app.suppliers` (sharing the Jinja2Templates instance with CSRF + flash context processors). Field set is `("name", "notes")` — no email/phone (locations are places, not contacts). Every mutation writes `entity_type="location"` audit row; updates record only the changed fields via `_diff`. Idempotent archive/unarchive (no audit row on no-op). Validation: empty/whitespace name = 400, duplicate name = 400 (active + archived share the namespace, mirroring suppliers). **Nav link "Locations" added** to the role-aware primary nav for Manager + Admin alongside "Suppliers". 46 new tests (6 unit `_locations` + 35 integration `locations_routes` + 5 layout for nav-locations) + 1 new e2e (manager promotion → create "Workshop Bench" → archive → unarchive). 198 unit+integration + 6 e2e passing. |
...
- **S2 / route + template duplication between `suppliers.py` and `locations.py` has hit the extraction threshold.** Two routers now share a verbatim `init_templates`/`_t` plumbing block, near-verbatim helper functions (`_normalise`/`_validate_name`/`_check_name_unique`/`_diff`/`_flash`), and a near-verbatim list/new/create/edit/update/archive/unarchive shape that differs only by entity model + field set. Templates are also ~80% structural duplicates. **The `init_templates` plumbing is the safe extraction** — pull a single shared `app.template_env` module exposing the Jinja2Templates instance (with CSRF + flash processors already installed), and have routers import from it directly. **The route helper duplication is a less safe extraction** — collapsing `_diff`/`_normalise` into a generic settings-CRUD helper would force a `_FIELDS` registry pattern that hides which fields each entity actually has. Plan: queue **S2.5** to do the templates extraction only, leave route handler duplication until S3 (taxonomy) lands and we can see three concrete shapes side-by-side. *(Supersedes the earlier "S1 / init_templates is a code smell" note.)*
